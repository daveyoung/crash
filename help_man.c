...
"NAME",
"       crash-alias - command aliases",
"",
"SYNOPSIS",
"       alias [alias] [command string]",
"",
"DESCRIPTION",
"       This command creates an alias for a given command string.  If no",
"       arguments are entered, the current list of aliases are displayed.  If",
"       one argument is entered, the command string for that alias, if any, is",
"       displayed.",
...
"       .crashrc file in the current directory.  Aliases in the .crashrc file in",
"       the current directory override those in $HOME/.crashrc.  Aliases entered",
"       with this command or by runtime input file override those defined in",
"       either .crashrc file.  Aliases may be deleted by entering an empty",
"       string for the second argument.  If redirection characters are to be",
"       part of the command string, the command string must be enclosed by",
"       quotation marks.",
"",
...
"value ...",
"NAME",
"       crash-ascii - translate a hexadecimal string to ASCII",
"",
"SYNOPSIS",
"       ascii value ...",
"",
...
"NAME",
"       crash-bpf - extended Berkeley Packet Filter (eBPF)",
"",
"SYNOPSIS",
...
"       maps.  With no arguments, basic information about each loaded eBPF",
"       program and map is displayed.  For each eBPF program, its ID number, the",
"       addresses of its bpf_prog and bpf_prog_aux data structures, its type,",
"       tag, and the IDs of the eBPF maps that it uses are displayed.  For each",
"       eBPF map, its ID number, the address of its bpf_map data structure, its",
"       type, and the hexadecimal value of its map_flags are displayed.",
"",
"       -p ID  displays the basic information specific to the program ID, plus",
"              the size in bytes of its translated bytecode, the size in bytes",
"              of its jited code, the number of bytes locked into memory, the",
"              time that the program was loaded, whether it is GPL compatible,",
"              its name string, and its UID.",
"",
...
"              size in bytes of its key and value, the maximum number of key-",
"              value pairs that can be stored within the map, the number of",
"              bytes locked into memory, its name string, and its UID.",
"",
...
"",
"       -s     with -p or -P, dump the bpf_prog and bpf_prog_aux data",
"              structures.  with -m or -M, dump the bpf_map structure.",
"",
...
"",
"       crash> bpf -p 20 -j ID     BPF_PROG       BPF_PROG_AUX   BPF_PROG_TYPE",
"       TAG        USED_MAPS 20 ffffbc00c0d41000 ffff9ff2610f8e00  CGROUP_SKB",
"       2a142ef67aaad174   19,20",
"            XLATED: 296  JITED: 229  MEMLOCK: 4096",
"            LOAD_TIME: Fri Apr 20 19:39:21 2018",
"            GPL_COMPATIBLE: yes  UID: 0",
"",
...
"",
"       To display all possible information that this command offers about all",
"       programs and maps, enter:",
"",
"       crash> bpf -PM -jTs",
"",
"SEE ALSO",
...
char *help_bt[] = {
"@MANPAGE@",
"[-a|-c cpu(s)|-g|-r|-t|-T|-l|-e|-E|-f|-F|-o|-O|-v|-p] [-R ref] [-s [-x|d]] [-I ip] [-S sp] [-n idle] [pid | task]",
"NAME",
"       crash-bt - backtrace",
"",
"SYNOPSIS",
"       bt [-a|-c cpu(s)|-g|-r|-t|-T|-l|-e|-E|-f|-F|-o|-O|-v|-p] [-R ref] [-s",
"       [-x|d]] [-I ip] [-S sp] [-n idle] [pid | task]",
"",
"DESCRIPTION",
"       Display a kernel stack backtrace.  If no arguments are given, the stack",
"       trace of the current context will be displayed.",
"",
"       -a     displays the stack traces of the active task on each CPU.  (only",
"              applicable to crash dumps)",
"",
"       -A     same as -a, but also displays vector registers (S390X only).",
"",
"       -n idle",
"              filter the stack of idle tasks (x86_64, arm64).  (only applicable",
"              to crash dumps)",
"",
"       -p     display the stack trace of the panic task only.  (only applicable",
"              to crash dumps)",
"",
"       -c cpu display the stack trace of the active task on one or more CPUs,",
"              which can be specified using the format \"3\", \"1,8,9\", \"1-23\", or",
"              \"1,8,9-14\". (only applicable to crash dumps)",
"",
...
"              frames.",
"",
"       -E     search the IRQ stacks (x86, x86_64, arm64, riscv64 and ppc64),",
"              and the exception stacks (x86_64) for possible exception frames;",
"              all other arguments except for -c will be ignored since this is",
"              not a context-sensitive operation.",
"",
...
"",
"       -F[F]  similar to -f, except that the stack data is displayed",
"              symbolically when appropriate; if the stack data references a",
"              slab cache object, the name of the slab cache will be displayed",
"              in brackets; on ia64, the substitution is done to the argument",
"              register contents.  If -F is entered twice, and the stack data",
"              references a slab cache object, both the address and the name of",
"              the slab cache will be displayed in brackets.",
"",
"       -v     check the kernel stack of all tasks for evidence of stack",
"              overflows.  It does so by verifying the thread_info.task pointer,",
"              ensuring that the thread_info.cpu is a valid cpu number, and",
"              checking the end of the stack for the STACK_END_MAGIC value.",
"",
"       -o     arm64: use optional backtrace method; not supported on Linux 4.14",
"              or later kernels.  x86: use old backtrace method, permissible",
"              only on kernels that were compiled without the -fomit-",
"              frame_pointer.  x86_64: use old backtrace method, which dumps",
"              potentially stale kernel text return addresses found on the",
"              stack.",
"",
"       -O     arm64: use optional backtrace method by default; subsequent usage",
"              of this option toggles the backtrace method.  x86: use old",
"              backtrace method by default, permissible only on kernels that",
"              were compiled without the -fomit-frame_pointer; subsequent usage",
"              of this option toggles the backtrace method.  x86_64: use old",
"              backtrace method by default; subsequent usage of this option",
"              toggles the backtrace method.",
"",
...
"",
"       -S sp  use sp as the starting stack frame address.  pid  displays the",
"              stack trace(s) of this pid.",
...
"",
"       Note that all examples below are for x86 only.  The output format will",
"       differ for other architectures.  x86 backtraces from kernels that were",
"       compiled with the --fomit-frame-pointer CFLAG occasionally will drop",
"       stack frames, or display a stale frame reference.  When in doubt as to",
"       the accuracy of a backtrace, the -t or -T options may help fill in the",
"       blanks.",
"",
"EXAMPLES",
...
"       From the extra frame data that is displayed, the arguments passed to",
"       each function can be determined.  Re-examining the PID 1592 trace above:",
"           crash> bt -f 1592",
"           PID: 1592   TASK: c0cec000  CPU: 3   COMMAND: \"httpd\"",
"            #0 [c0ceded4] schedule at c011af85",
...
"       insmod the sample \"crash.c\" kernel module that comes as part of the Red",
"       Hat netdump package:",
"          crash> bt",
...
"",
"       Display the symbol name plus its offset in each frame, overriding the",
"       current output format with hexadecimal:",
"           crash> bt -sx",
...
"",
"       The following three examples show the difference in the display of the",
"       same stack frame's contents using -f, -F, and -FF:",
"           crash> bt -f",
"           ...",
...
"[cpu]",
"NAME",
"       crash-cpu - set context to the active task on a cpu",
"",
"SYNOPSIS",
"       cpu [cpu]",
"",
"DESCRIPTION",
"       This command sets the current context to the active task on a cpu.  If",
"       no argument is given, the current context is displayed.",
...
"NAME",
"       crash-dev - device data",
"",
"SYNOPSIS",
"       dev [-i | -p | -d | -D ] [-V | -v index [file]]",
"",
"DESCRIPTION",
"       If no argument is entered, this command dumps character and block device",
"       data.",
"",
...
"",
"       -d     display disk I/O statistics: TOTAL: total number of allocated in-",
"              progress I/O requests SYNC: I/O requests that are synchronous",
"              ASYNC: I/O requests that are asynchronous READ: I/O requests that",
"              are reads (older kernels) WRITE: I/O requests that are writes",
"              (older kernels) DRV: I/O requests that are in-flight in the",
"              device driver.  If the device driver uses blk-mq interface, this",
"              field shows N/A(MQ).  If not available, this column is not shown.",
"",
"       -D     same as -d, but filter out disks with no in-progress I/O",
"              requests.",
"",
"       If the dumpfile contains device dumps:",
"",
"       -V     display an indexed list of all device dumps present in the",
"              vmcore, showing their file offset, size and name.",
"",
"       -v index",
"              select and display one device dump based upon an index value",
...
"              options may be used to further tailor the output.  file  only",
"              used with -v, copy the device dump data to a file.",
"",
"EXAMPLES",
...
"",
"       Format and display a device's dump data to the screen using the \"rd\"",
"       command:",
...
"",
"DESCRIPTION",
"       This command disassembles source code instructions starting (or ending)",
"       at a text address that may be expressed by value, symbol or expression:",
"",
"       -r     (reverse) displays all instructions from the start of the routine",
"              up to and including the designated address.",
"",
"       -f     (forward) displays all instructions from the given address to the",
"              end of the routine.",
"",
...
"",
"       -s     displays the filename and line number of the source code that is",
"              associated with the specified text location, followed by a source",
"              code listing if it is available on the host machine.  The line",
"              associated with the text location will be marked with an",
"              asterisk; depending upon gdb's internal \"listsize\" variable,",
"              several lines will precede the marked location. If a \"count\"",
"              argument is entered, it specifies the number of source code lines",
"              to be displayed after the marked location; otherwise the",
"              remaining source code of the containing function will be",
"              displayed.",
"",
"       -b [num]",
"              modify the pre-calculated number of encoded bytes to skip after a",
"              kernel BUG (\"ud2a\") instruction; with no argument, displays the",
"              current number of bytes being skipped. (x86 and x86_64 only)",
"              address  starting hexadecimal text address.  symbol  symbol of",
"              starting text address.  On ppc64, the symbol preceded by '.' is",
"              used.",
"",
...
"       is an integral number of gigabytes, megabytes, or kilobytes, a short-",
"       hand translation of the number will also be shown next to the",
"       hexadecimal value.  If the most significant bit is set, the decimal",
"       display will show both unsigned and signed (negative) values.",
"       Expressions must of the format (x operator y), where \"x\" and \"y\" may be",
"       either numeric values or symbols.  The list of operators are:",
"                            +  -  &  |  ^  *  %  /  <<  >>",
"",
...
"       when obvious.  Numbers may be followed by the letters \"k\" or \"K\", \"m\" or",
"       \"M\", and \"g\" or \"G\", which multiplies the value by a factor of 1024, 1",
"       megabyte or 1 gigabyte, respectively.  Numeric arguments may be preceded",
"       by the one's complement operator ~.",
"",
"       -b     Indicate which bit positions in the resultant value are set.",
"",
"       -l     Numeric arguments are presumed to be 64-bit values, and the",
"              result will be expressed as a 64-bit value. (ignored on 64-bit",
"              processors) However, if either operand or the resultant value are",
"              64-bit values, then the result will be also be expressed as a",
"              64-bit value.",
"",
...
"NAME",
"       crash-extend - extend the crash command set",
"",
"SYNOPSIS",
...
"",
"       -u shared-object",
"              unload the specified shared object file; if no file arguments are",
"              specified, unload all objects.",
"",
...
"       \"help\" command screen.",
"",
"       An example of a shared object prototype file, and how to compile it into",
"       a shared object, is appended below.",
"",
"EXAMPLES",
...
"       turn calls the shared object's constructor function.  The shared",
"       object's constructor function should register its command set by calling",
...
"       the command's function address, a pointer to an array of help data",
"       strings, and a flags field.  The help_data field is optional; if it is",
"       non-NULL, it should point to an array of character strings used by the",
"       \"help\" command, and during command failures.  The flags field currently",
"       has two available bit settings, REFRESH_TASK_TABLE, which should be set",
"       if it is preferable to reload the current set of running processes just",
"       prior to executing the command (on a live system) and MINIMAL, which",
"       should be set if the command should be available in minimal mode.",
"       Terminate the array of command_table_entry structures with an entry with",
"       a NULL command name.",
"",
...
"       Note the comments contained within it for further details.  Cut and",
"       paste the following output into a file, and call it, for example,",
"       \"echo.c\".  Then compiled in either of two manners.  Either manually like",
"       so:",
"",
"       gcc -shared -rdynamic -o echo.so echo.c -fPIC -D<machine-type>",
"       $(TARGET_CFLAGS)",
"",
"       where <machine-type> must be one of the MACHINE_TYPE #define's in",
"       defs.h, and where $(TARGET_CFLAGS) is the same as it is declared in the",
"       top-level Makefile after a build is completed.  Or alternatively, the",
"       \"echo.c\" file can be copied into the \"extensions\" subdirectory, and",
"       compiled automatically like so:",
"",
"       make extensions",
"",
...
"",
"       void echo_init(void);    /* constructor function */ void",
"       echo_fini(void);    /* destructor function (optional) */",
"",
...
"       };",
"",
"       void __attribute__((constructor)) echo_init(void) /* Register the",
"       command set. */ {",
"               register_extension(command_table);",
"",
"       }",
"",
"       /* *  This function is called if the shared object is unloaded.  *  If",
"       desired, perform any cleanups here.  */ void __attribute__((destructor))",
"       echo_fini(void) { }",
"",
"       /* *  Arguments are passed to the command functions in the global",
"       args[argcnt] *  array.  See getopt(3) for info on dash arguments.  Check",
"       out defs.h and *  other crash commands for usage of the myriad of",
"       utility routines available *  to accomplish what your task.  */ void",
"       cmd_echo(void) {",
"               int c;",
"",
...
"       }",
"",
"       /* *  The optional help data is simply an array of strings in a defined",
"       format.  *  For example, the \"help echo\" command will use the",
"       help_echo[] string *  array below to create a help page that looks like",
"       this: * *    NAME *      echo - echoes back its arguments * *",
"       SYNOPSIS *      echo arg ...  * *    DESCRIPTION *      This command",
"       simply echoes back its arguments.  * *    EXAMPLE *      Echo back all",
"       command arguments: * *        crash> echo hello, world *        hello,",
"       world * */",
"",
"       char *help_echo[] = {",
"               \"echo\",                        /* command name */",
...
"",
"SYNOPSIS",
"       files [-d dentry] | [-p inode] | [-n inode]|[-c] [-R reference] [pid |",
"       taskp] ...",
"",
"DESCRIPTION",
"       This command displays information about open files of a context.  It",
"       prints the context's current root directory and current working",
"       directory, and then for each open file descriptor it prints a pointer to",
"       its file struct, a pointer to its dentry struct, a pointer to the inode,",
"       the file type, and the pathname.  If no arguments are entered, the",
"       current context is used.  The -R option, typically invoked from \"foreach",
"       files\", searches for references to a supplied number, address, or",
"       filename argument, and prints only the essential information leading up",
"       to and including the reference.  The -d option is not context specific,",
"       and only shows the data requested.",
"",
"       -d dentry",
"              given a hexadecimal dentry address, display its inode, super",
"              block, file type, and full pathname.",
"",
"       -p inode",
"              given a hexadecimal inode address, dump all of its pages that are",
"              in the page cache.",
"",
"       -n inode",
"              given a hexadecimal inode address, check all the pages in the",
"              page cache, and display a NUMA node distribution.",
"",
"       -c     for each open file descriptor, prints a pointer to its inode, a",
"              pointer to the inode's i_mapping address_space structure, the",
"              number of pages of the inode that are in the page cache, the file",
"              type, and the pathname.",
"",
"       -R reference",
"              search for references to this file descriptor number, filename,",
"              dentry, inode, address_space, or file structure address.  pid  a",
"              process PID.  taskp  a hexadecimal task_struct pointer.",
"",
"EXAMPLES",
...
"",
"       For each open file, display the number of pages that are in the page",
"       cache:",
"           crash> files -c 1954",
...
"           crash> files -n ffff8898857c55a8",
"",
"       INODE NRPAGES",
"",
...
char *help_foreach[] = {
"@MANPAGE@",
"[[pid | taskp | name | state | [kernel | user | gleader]] ...] command [flag] [argument]",
"NAME",
"       crash-foreach - display command data for multiple tasks in the system",
"",
"SYNOPSIS",
...
"",
"       gleader  perform the command(s) on all user (non-kernel) thread group",
"       leaders.",
...
"       pre-pended before the command output for each selected task.  Consult",
"       the help page of each of the command types above for details.",
"",
"EXAMPLES",
...
"[pathname | inode]",
"NAME",
"       crash-fuser - file users",
"",
"SYNOPSIS",
"       fuser [pathname | inode]",
"",
"DESCRIPTION",
"       This command displays the tasks using specified files or sockets.  Tasks",
"       will be listed that reference the file as the current working directory,",
"       root directory, an open file descriptor, or that mmap the file.  If the",
"       file is held open in the kernel by the lockd server on behalf of a",
"       client discretionary file lock, the client hostname is listed.",
...
"command ...",
"NAME",
"       crash-gdb - gdb command",
"",
"SYNOPSIS",
"       gdb command ...",
"",
"DESCRIPTION",
"       This command passes its arguments directly to gdb for processing.  This",
"       is typically not necessary, but where ambiguities between crash and gdb",
...
"       mode where all commands are passed directly to gdb.  When running in",
"       that mode, native crash commands may be executed by preceding them with",
"       the \"crash\" directive.  To restore native crash mode, enter \"set gdb",
"       off\".",
"",
"EXAMPLES",
"           crash> gdb help",
...
"       page for the command is displayed.  If \"all\" is entered, help pages for",
"       all commands will be displayed.  If neither of the above is entered, the",
"       argument string will be passed on to the gdb help command.",
"",
"       A number of internal debug, statistical, and other dumpfile related data",
...
"NAME",
"       crash-ipcs - System V IPC facilities",
"",
"SYNOPSIS",
...
"       no arguments, the command will display kernel usage of all three",
"       facilities.",
"",
...
"",
"       For kernels supporting namespaces, the -n option may be used to display",
"       the IPC facilities with respect to the namespace of a specified task:",
"",
"       -n pid a process PID.",
"",
"       -n task",
"              a hexadecimal task_struct pointer.",
"",
"EXAMPLES",
...
"",
"       Display the shared memory data associated with shmid_kernel",
"       ffff880473a28250:",
...
"       consolidated per-IRQ display.  For kernel versions 2.6.37 and later the",
"       display consists of the irq_desc/irq_data address, its irqaction",
"       address(es), and the irqaction name strings.  Alternatively, the intel",
"       interrupt descriptor table, bottom half data, cpu affinity for in-use",
"       irqs, or kernel irq stats may be displayed.  If no index value",
...
"",
"       -s     dump the kernel irq stats; if no cpu specified with -c, the irq",
"              stats of all cpus will be displayed.",
"",
...
char *help_kmem[] = {
"@MANPAGE@",
"[-f|-F|-c|-C|-i[=shared]|-v|-V|-n|-z|-o|-h] [-p | -m member[,member]] [[-s|-S|-S=cpu[s]|-r] [slab] [-I slab[,slab]]] [-g [flags]] [[-P] address]]",
"NAME",
"       crash-kmem - kernel memory",
"",
"SYNOPSIS",
"       kmem [-f|-F|-c|-C|-i[=shared]|-v|-V|-n|-z|-o|-h] [-p | -m",
"       member[,member]] [[-s|-S|-S=cpu[s]|-r] [slab] [-I slab[,slab]]] [-g",
"       [flags]] [[-P] address]]",
"",
"DESCRIPTION",
"       This command displays information about the use of kernel memory.",
"",
"       -f     displays the contents of the system free memory headers.  also",
"              verifies that the page count equals nr_free_pages.",
"",
...
"",
"       -i     displays general memory usage information.  Note: SHARED is no",
"              longer printed (too slow on large memory systems) unless",
"              specifying \"-i=shared\".",
"",
...
"              \"P\": SECTION_MARKED_PRESENT \"M\": SECTION_HAS_MEM_MAP \"O\":",
"              SECTION_IS_ONLINE \"E\": SECTION_IS_EARLY \"D\":",
...
"",
"       -p     displays basic information about each page structure in the",
"              system mem_map[] array, made up of the page struct address, its",
"              associated physical address, the page.mapping, page.index,",
"              page._count and page.flags fields.",
"",
"       -m member",
"              similar to -p, but displays page structure contents specified by",
"              a comma-separated list of one or more struct page members.  The",
"              \"flags\" member will always be expressed in hexadecimal format,",
"              and the \"_count\" and \"_mapcount\" members will always be expressed",
"              in decimal format.  Otherwise, all other members will be",
"              displayed in hexadecimal format unless the output radix is 10 and",
"              the member is a signed/unsigned integer.  Members that are data",
"              structures may be specified either by the data structure's member",
"              name, or expanded to specify a member of the data structure.  For",
"              example, \"-m lru\" refers to a list_head data structure, and both",
"              the list_head.next and list_head.prev pointer values will be",
"              displayed, whereas if \"-m lru.next\" is specified, just the",
"              list_head.next value will be displayed.",
"",
"       -s     displays basic kmalloc() slab data.",
"",
"       -S     displays all kmalloc() slab data, including all slab objects, and",
"              whether each object is in use or is free.  If CONFIG_SLUB, slab",
"              data for each per-cpu slab is displayed, along with the address",
"              of each kmem_cache_node, its count of full and partial slabs, and",
"              a list of all tracked slabs.  Note: one can specify the per-cpu",
"              slab data to be displayed; the cpu[s] can be given as \"1,3,5\",",
"              \"1-3\", \"1,3,5-7,10\", \"all\", or \"a\" (shortcut for \"all\").",
"",
"       -r     displays the accumulated basic kmalloc() slab data of each root",
"              slab cache and its children.  The kernel must contain the",
...
"",
"       -I slab",
"              when used with -s, -S or -r, one or more slab cache names in a",
...
"           ...",
"",
"       Find the two pages that link to the page at ffffea0001dafb20 via their",
"       page.lru list_head's next and prev pointers:",
...
"",
"       Find all of the combined slab/page structures that are used by the",
...
char *help__list[] = {
"@MANPAGE@",
"[[-o] offset][-e end][-[s|S] struct[.member[,member] [-l offset]] -[x|d]] [-r|-B] [-h [-O head_offset]|-H] start",
"NAME",
"       crash-list - linked list",
"",
"SYNOPSIS",
"       list [[-o] offset][-e end][-[s|S] struct[.member[,member] [-l offset]]",
"       -[x|d]] [-r|-B] [-h [-O head_offset]|-H] start",
"",
"DESCRIPTION",
"       This command dumps the contents of a linked list.  The entries in a",
"       linked list are typically data structures that are tied together in one",
"       of two formats:",
"",
"       1. A starting address points to a data structure; that structure",
"       contains",
...
"",
"       2. Most Linux lists of data structures are doubly-linked using",
"       \"list_head\"",
...
"",
"       This command can handle both types of linked list; in both cases the",
"       list of addresses that are dumped are the addresses of the data",
"       structures themselves.",
"",
"       Alternatively, the address of a list_head, or other similar list linkage",
"       structure whose first member points to the next linkage structure, may",
"       be used as the starting address.  The caveat with this type of usage is",
"       that the list may pass through, and display the address of, an external",
"       standalone list head which is not an address of a list linkage structure",
"       that is embedded within the data structure of interest.",
"",
...
"                         virtual address.",
"",
"       -e end If the list ends in a manner unlike the typical manners that are",
"              described above, an explicit ending address value may be entered.",
"",
"       -s struct",
"              For each address in list, format and print as this type of",
...
"              structure member contains an embedded structure or is an array,",
"              the output may be restricted to the embedded structure or an",
"              array element by expressing the struct argument as",
"              \"struct.member.member\" or \"struct.member[index]\"; embedded member",
"              specifications may extend beyond one level deep by expressing the",
"              argument as \"struct.member.member.member...\".",
"",
"       -S struct",
"              Similar to -s, but instead of parsing gdb output, member values",
"              are read directly from memory, so the command works much faster",
"              for 1-, 2-, 4-, and 8-byte members.",
"",
"       -O offset",
"              Only used in conjunction with -h; it specifies the offset of head",
"              node list_head embedded within a data structure which is",
"              different than the offset of list_head of other nodes embedded",
"              within a data structure.  The offset may be entered in either of",
"              the following manners:",
"                               1. in \"structure.member\" format.",
"                               2. a number of bytes.",
"",
"       -l offset",
"              Only used in conjunction with -s, if the start address argument",
...
"              structure), the offset to the embedded member may be entered in",
...
"",
"       -r     For a list linked with list_head structures, traverse the list in",
"              the reverse order by using the \"prev\" pointer instead of \"next\".",
"",
...
"       symbolically, in hexadecimal format, or an expression evaluating to an",
"       address, depends upon whether the -h or -H option is pre-pended:",
...
"",
"       -H start",
"              The address of a list_head structure, typically that of an",
"              external, standalone LIST_HEAD().  The list typically ends when",
"              the embedded \"list_head.next\" of a data structure in the linked",
"              list points back to this \"start\" address.",
"",
"       -h start",
"              The address of a data structure which contains an embedded",
"              list_head.  The list typically ends when the embedded",
"              \"list_head.next\" of a data structure in the linked list points",
"              back to the embedded list_head contained in the data structure",
"              whose address is this \"start\" argument.",
"",
"       WARNING",
"",
"       When the \"-h start\" option is used, it is possible that the list_head-",
"       linked list will:",
...
"",
"       When that occurs, the data structure address displayed for that",
"       list_head will be incorrect, because the \"-h start\" option presumes that",
"       all list_head structures in the list are contained within the same type",
"       of data structure.  Furthermore, if the \"-s struct[.member[,member]\"",
"       option is used, it will display bogus data for that particular",
"       list_head.",
"",
"       A similar issue may be encountered when the \"start\" address is an",
"       embedded list_head or similar linkage structure whose first member",
"       points to the next linkage structure.  When that occurs, the address of",
"       any external list head will not be distinguishable from the addresses",
"       that are embedded in the data structure of interest.  Furthermore, if",
"       the \"-s\" and \"-l\" options are used, it will display bogus structure data",
"       when passing through any external list head structure that is not",
"       embedded in the specified data structure type.",
"",
"EXAMPLES",
...
"           c0252000",
"",
"       Given that the \"task_struct.p_pptr\" offset is 136 bytes, the same result",
...
"       struct file_system_type pointer named \"file_systems\", and linked by the",
"       \"next\" field in each file_system_type structure.  The following sequence",
"       displays the structure address followed by the name and fs_flags members",
"       of each registered file system type:",
...
"",
"       The run queue linking is done with the \"run_list\" member of the",
"       task_struct:",
...
"           ...",
"",
"       The task_struct of every task in the system is linked into a circular",
"       list by its embedded \"tasks\" list_head.  Show the task_struct addresses",
"       and the pids of all tasks in the system using \"-h\" option, starting with",
"       the task_struct at ffff88012b98e040:",
...
"             pid = 14183",
"",
"       Similar to the above, display the embedded sched_entity structure's",
"       on_rq member from each task_struct in the system:",
...
"       the address of a list_head or similar linkage structure whose first",
"       member points to the next linkage structure.  Again using the",
"       task_struct.tasks embedded list_head, dump the \"comm\" member of all",
"       tasks by using -l in conjunction with -s option:",
...
"             comm = \"crash\"",
"",
"       To display a linked list whose head node and other nodes are embedded",
"       within either same or different data structures resulting in different",
"       offsets for head node and other nodes, e.g. dentry.d_subdirs and",
"       dentry.d_child, the",
"",
...
"[-TtdmascR]",
"NAME",
"       crash-log - dump system message buffer",
"",
"SYNOPSIS",
"       log [-TtdmascR]",
"",
...
"       log entry's header.",
"",
"       -T     Display the message text with human readable timestamp.  (Be",
"              aware that the timestamp could be inaccurate!  The timestamp is",
...
"              to the variable-length record format.",
"",
...
"              For the variable-length record format, the level will be",
"              displayed in hexadecimal.  In older kernels, by default, the",
"              facility/flag bits will be stripped to only show the level, but",
"              if needed, can still be shown with 'set debug 1'.",
"",
"       -a     Dump the audit logs remaining in kernel audit buffers that have",
"              not been copied out to the user-space audit daemon.",
"",
"       -s     Dump the printk logs remaining in kernel safe per-CPU buffers",
"              that have not been flushed out to log_buf.",
"",
"       -c     Display the caller id field that identifies either the thread id",
"              or the CPU id (if in CPU context) that called printk(), if",
"              available.  Generally available on Linux 5.1 to 5.9 kernels",
"              configured with CONFIG_PRINTK_CALLER or Linux 5.10 and later",
"              kernels.",
"",
"       -R     Display the message text with human readable Rust symbol name if",
"              any, otherwise still print the original message text.",
"",
"EXAMPLES",
...
"           ...",
"",
"       On a system with the variable-length record format, and whose log_buf",
"       has been filled and wrapped around, display the log with timestamp data:",
"           crash> log",
...
"           ...",
"",
"       On a system which has printk_safe_seq_buf buffer, display its unflushed",
"       log with buffer name:",
"           crash> log",
"           ...",
...
"[-m | -c -[xd] | -o]",
"NAME",
"       crash-mach - machine specific data",
"",
"SYNOPSIS",
"       mach [-m | -c -[xd] | -o]",
"",
...
"              Display each cpu's x8664_pda structure (x86_64 only), Display the",
"              hwrpb_struct, and each cpu's percpu_struct (alpha only).",
"",
"       -x     override default output format with hexadecimal format.",
"",
"       -d     override default output format with decimal format.",
...
"[-a][-f [filename]]",
"NAME",
"       crash-map - store KVM dumpfile memory map data",
"",
"SYNOPSIS",
"       map [-a][-f [filename]]",
"",
"DESCRIPTION",
"       The layout of KVM guest dumpfiles created with \"virsh dump\" does not",
"       allow the crash utility to access the system's memory in a random access",
"       manner.  Therefore, during crash session initialization, a potentially",
"       time-consuming dumpfile scan procedure is required to create a physical-",
"       memory-to-file-offset map for use during the session.",
"",
"       This command may be used to append the memory map data to the dumpfile",
"       or to store it in a permanent file.  After this has been done,",
"       subsequent crash sessions using the dumpfile will no longer require the",
"       scan procedure:",
"",
"       -a     Append the memory map to the end of the KVM dumpfile.",
"",
"       -f     Create a memory map file.  If no filename argument is entered,",
"              the filename will consist of the dumpfile name with \".map\"",
"              appended, and will be located in the same directory as the",
"              dumpfile; it will be recognized and used automatically during",
"              subsequent crash sessions.  However, if a \"filename\" argument is",
"              entered, and the default location and naming convention are not",
"              used, then the new memory map file will have to be added to the",
"              crash command line during invocation.",
"",
"EXAMPLES",
"           crash> map",
...
"NAME",
"       crash-mod - module information and loading of symbols and debugging data",
"",
"SYNOPSIS",
...
"       currently installed modules, consisting of the module address, name,",
"       base address, size, the object file name (if known), and whether the",
"       module was compiled with CONFIG_KALLSYMS.",
"",
"       The arguments are concerned with with the loading or deleting of",
"       symbolic and debugging data from a module's object file.  A modules's",
"       object file always contains symbolic data (symbol names and addresses),",
"       but contains debugging data only if the module was compiled with the -g",
"       CFLAG.  In addition, the module may have compiled with CONFIG_KALLSYMS,",
"       which means that the module's symbolic data will have been loaded into",
"       the kernel's address space when it was installed.  If the module was not",
"       compiled with CONFIG_KALLSYMS, then only the module's exported symbols",
"       will be loaded into the kernel's address space.  Therefore, for the",
"       purpose of this command, it should noted that a kernel module may have",
"       been compiled in one of following manners:",
"",
"       1. If the module was built without CONFIG_KALLSYMS and without the -g",
"       CFLAG,",
"            then the loading of the module's additional non-exported symbols can",
"            be accomplished with this command.",
"",
"       2. If the module was built with CONFIG_KALLSYMS, but without the -g",
"       CFLAG,",
...
"",
"       3. If the module was built with CONFIG_KALLSYMS and with the the -g",
"       CFLAG,",
"            then the loading of the module's debugging data can be accomplished",
"            with this command.",
"",
"       4. If the module was built without CONFIG_KALLSYMS but with the -g",
"       CFLAG,",
"            then the loading of the both module's symbolic and debugging data can",
"            be accomplished with this command.",
"",
"       -s module [objfile]",
"              Loads symbolic and debugging data from the object file for the",
"              module specified.  If no objfile argument is appended, a search",
"              will be made for an object file consisting of the module name",
"              with a .o or .ko suffix, starting at the /lib/modules/<release>",
"              directory on the host system, or if not found there, starting at",
"              the directory containing the kernel namelist file.  If an objfile",
"              argument is appended, then that file will be used.",
"",
"       -d module",
"              Deletes the symbolic and debugging data of the module specified.",
"",
"       -S [directory]",
"              Load symbolic and debugging data from the object file for all",
"              loaded modules.  For each module, a search will be made for an",
"              object file consisting of the module name with a .o or .ko",
"              suffix, starting at the /lib/modules/<release> directory of the",
"              host system, or if not found there, starting at the directory",
"              containing the kernel namelist file.  If a directory argument is",
"              appended, then the search will be restricted to that directory.",
"",
"       -D     Deletes the symbolic and debugging data of all modules.",
"",
"       -t     Display the contents of the module's \"taints\" bitmask if it is",
"              non-zero.  When possible, the \"taints\" bits are translated to",
"              symbolic letters of the taint type; otherwise the hexadecimal",
"              value is shown.  In older kernels, the contents of the",
"              \"license_gplok\" field is displayed in hexadecimal; the field may",
"              be either a bitmask or a boolean, depending upon the kernel",
"              version.  The relevant kernel sources should be consulted for the",
"              meaning of the letter(s) or hexadecimal bit value(s).  For",
"              modules that have a \"gpgsig_ok\" field that is zero (unsigned),",
"              the notation \"(U)\" is shown.",
"",
"       -r     Passes the -readnow flag to the embedded gdb module, which will",
"              override the two-stage strategy that it uses for reading symbol",
"              tables from module object files.",
"",
"       -R     Reinitialize module data. All currently-loaded symbolic and",
"              debugging data will be deleted, and the installed module list",
"              will be updated (live system only).",
"",
"       -g     When used with -s or -S, add a module object's section start and",
"              end addresses to its symbol list.",
"",
...
"       /lib/modules/<release> will be overridden as the default directory tree",
"       that is searched for module object files.",
"",
"       After symbolic and debugging data have been loaded, backtraces and text",
"       disassembly will be displayed appropriately.  Depending upon the",
"       processor architecture, data may also printed symbolically with the \"p\"",
"       command; at a minimum, the \"rd\" command may be used with module data",
"       symbols.",
"",
"       If crash can recognize that the set of modules has changed while running",
"       a session on a live kernel, the module data will be reinitialized the",
"       next time this command is run; the -r option forces the",
"       reinitialization.",
"",
"EXAMPLES",
...
"           ...",
"",
"       Display the currently-installed modules on a system where all modules",
"       were compiled with CONFIG_KALLSYMS:",
"           crash> mod",
"",
"       MODULE NAME BASE SIZE OBJECT FILE",
...
"           ...",
"",
"       Load the symbolic and debugging data of the dm_mod module from its known",
"       location:",
"           crash> mod -s dm_mod",
"",
"       MODULE NAME BASE SIZE OBJECT FILE",
"           f7e44c20  dm_mod          f7e34000   88568  /lib/modules/2.6.32/kernel/drivers/md/dm-mod.ko",
"",
"       Delete the current symbolic and debugging data of the dm_mod module, and",
"       then re-load it from a specified object file:",
"           crash> mod -d dm_mod",
...
"           ...",
"",
"       Display modules that are \"tainted\", where in this case where they are",
...
"NAME",
"       crash-mount - mounted filesystem data",
"",
"SYNOPSIS",
"       mount [-f][-i] [-n pid|task]",
"       [mount|vfsmount|superblock|dev|dir|dentry|inode]",
"",
"DESCRIPTION",
"       This command displays basic information about the currently-mounted",
"       filesystems.  The per-filesystem dirty inode list or list of open files",
"       for the filesystem may also be displayed.",
"",
...
"",
"       For kernels supporting namespaces, the -n option may be used to display",
"       the mounted filesystems with respect to the namespace of a specified",
"       task:",
"",
"       -n pid a process PID.",
"",
...
"",
"       The first column of the command output displays the filesystem's",
"       vfsmount structure address for kernels prior to Linux 3.3.  For Linux",
"       3.3 and later kernels, the first column displays the filesystem's mount",
"       structure address, which contains an embedded vfsmount structure.",
"",
"EXAMPLES",
...
"NAME",
"       crash-net - network command",
"",
"SYNOPSIS",
"       net [[-s | -S] [-xd] [-R ref] [pid | task]] [-a] [ -n [pid | task]] [-N",
"       addr]",
"",
"DESCRIPTION",
...
"       addresses are displayed.  For kernels supporting namespaces, the -n",
"       option may be used to display the list of network devices with respect",
"       to the network namespace of a current context or a task specified by pid",
"       or task:",
"",
...
"              a hexadecimal task_struct pointer.",
"",
"       The -s and -S options display data with respect to the current context,",
"       but may be appended with an argument to show the socket data with",
"       respect to a specified task:",
"",
"       -s     display open network socket/sock addresses, their family and",
"              type, and for INET and INET6 families, their source and",
"              destination addresses and ports.",
"",
"       -s pid same as above, for task with process PID pid.",
"",
"       -s task",
"              same as above, for task with hexadecimal task_struct pointer",
"              task.",
"",
"       -S     displays open network socket/sock addresses followed by a dump of",
"              both data structures.",
"",
"       -S pid same as above, with respect to process PID.",
"",
"       -S task",
"              same as above, with respect to hexadecimal task_struct pointer.",
"",
...
"       sock address, or a file descriptor; if found, only the referenced fd,",
"       socket or sock data will be displayed:",
"",
...
"",
"       -N addr",
"              translates an IPv4 address expressed as a decimal or hexadecimal",
...
"",
"       Display the network device list with respect to the network namespace of",
"       PID 2618:",
"           crash> net -n 2618",
"              NET_DEVICE     NAME   IP ADDRESS(ES)",
...
"NAME",
"       crash-p - print the value of an expression",
"",
"SYNOPSIS",
"       p [-x|-d][-u] [expression | symbol[:cpuspec]]",
"",
"DESCRIPTION",
"       This command passes its arguments on to gdb \"print\" command for",
"       evaluation.",
...
"",
"       The default output format is decimal, but that can be changed at any",
"       time with the two built-in aliases \"hex\" and \"dec\".  Alternatively,",
"       there are two other built-in aliases, \"px\" and \"pd\", which force the",
"       command output to be displayed in hexadecimal or decimal, without",
...
"           }",
"",
"       If a per-cpu symbol is entered as a argument, its data type and all of",
"       its per-cpu addresses are displayed:",
"           crash> p irq_stat",
"           PER-CPU DATA TYPE:",
...
"             [3]: ffff88021e2d1540",
"",
"       To display the contents a per-cpu symbol for CPU 1, append a cpu-",
"       specifier:",
"           crash> p irq_stat:1",
...
"NAME",
"       crash-pointer - pointer-to short-cut",
"",
"SYNOPSIS",
"       * (struct or union command arguments)",
"",
"DESCRIPTION",
"       This command is a short-cut command that replaces the requirement to",
"       enter \"struct\" or \"union\" command names.  For details on the arguments",
"       to those commands, enter \"help struct\" or \"help union\".",
"",
"EXAMPLES",
...
char *help_ps[] = {
"@MANPAGE@",
"[-k|-u|-G|-I|-y policy] [-s] [-p|-c|-t|-[l|m][-C cpu]|-a|-g|-r|-S|-A|-H|-Y] [pid | task | command] ...",
"NAME",
"       crash-ps - display process status information",
"",
"SYNOPSIS",
"       ps [-k|-u|-G|-I|-y policy] [-s] [-p|-c|-t|-[l|m][-C",
"       cpu]|-a|-g|-r|-S|-A|-H|-Y] [pid | task | command] ...",
"",
"DESCRIPTION",
"       This command displays process status for selected, or all, processes in",
"       the system.  If no arguments are entered, the process data is is",
"       displayed for all processes.  Specific processes may be selected by",
"       using the following identifier formats:",
"              pid  a process PID.",
...
"",
"       -y policy",
"              restrict the output to tasks having a specified scheduling policy",
"              expressed by its integer value or by its (case-insensitive) name;",
"              multiple policies may be entered in a comma-separated list: 0 or",
"              NORMAL 1 or FIFO 2 or RR 3 or BATCH 4 or ISO 5 or IDLE 6 or",
"              DEADLINE",
"",
...
"       \"offline\" is set to \"hide\", the active task on an offline CPU will be",
"       highlighted by a \"-\" preceding its information.",
"",
"       Alternatively, information regarding parent-child relationships, per-",
"       task time usage data, argument/environment data, thread groups, or",
"       resource limits may be displayed:",
"",
"       -p     display the parental hierarchy of selected, or all, tasks.",
"",
"       -c     display the children of selected, or all, tasks.",
"",
"       -t     display the task run time, start time, and cumulative user and",
"              system times.",
"",
...
"              hours, minutes, seconds, and milliseconds since the task was last",
"              run on a cpu.",
"",
"       -C cpus",
"              only usable with the -l or -m options, dump the timestamp data in",
"              per-cpu blocks, where the cpu[s] can be specified as \"1,3,5\",",
"              \"1-3\", \"1,3,5-7,10\", \"all\", or \"a\" (shortcut for \"all\").",
"",
...
"",
"       Display the tasks in the thread group for each instance of the program",
"       named \"multi-thread\":",
...
"contents ...",
"NAME",
"       crash-pte - translate a page table entry",
"",
"SYNOPSIS",
"       pte contents ...",
"",
"DESCRIPTION",
"       This command translates the hexadecimal contents of a PTE into its",
"       physical page address and page bit settings.  If the PTE references a",
"       swap location, the swap device and offset are displayed.",
"",
"EXAMPLES",
"           crash> pte d8e067",
"",
...
"NAME",
"       crash-ptov - physical to virtual",
"                per-cpu to virtual",
"",
"SYNOPSIS",
...
"       virtual address.  Alternatively, a hexadecimal per-cpu offset and cpu",
"       specifier will be translated into kernel virtual addresses for each cpu",
"       specified.",
...
"",
"       Translate per-cpu offset b0c0 into a kernel virtual address for all",
"       cpus:",
"           crash> ptov b0c0:a",
"           PER-CPU OFFSET: b0c0",
"",
...
char *help_rd[] = {
"@MANPAGE@",
"[-adDsSupxmfNR][-8|-16|-32|-64][-o offs][-e addr][-r file][address|symbol] [count]",
"NAME",
"       crash-rd - read memory",
"",
"SYNOPSIS",
"       rd [-adDsSupxmfNR][-8|-16|-32|-64][-o offs][-e addr][-r",
"       file][address|symbol] [count]",
"",
"DESCRIPTION",
"       This command displays the contents of memory, with the output formatted",
"       in several different manners.  The starting address may be entered",
"       either symbolically or by address.  The default output size is the size",
"       of a long data type, and the default output format is hexadecimal.  When",
"       hexadecimal output is used, the output will be accompanied by an ASCII",
"       translation.",
"",
...
"",
"       -D     display output in unsigned decimal format (default is",
"              hexadecimal).",
"",
"       -s     displays output symbolically when appropriate.",
"",
"       -S[S]  displays output symbolically when appropriate; if the memory",
"              contents reference a slab cache object, the name of the slab",
"              cache will be displayed in brackets.  If -S is entered twice, and",
"              the memory contents reference a slab cache object, both the",
"              memory contents and the name of the slab cache will be displayed",
"              in brackets.",
"",
...
"",
"       -a     display output in ASCII characters if the memory contains",
"              printable ASCII characters; if no count argument is entered, stop",
"              at the first non-printable character.",
"",
"       -N     display output in network byte order (only valid for 16- and",
"              32-bit values)",
"",
"       -R     display memory in reverse order; memory will be displayed up to",
"              and including the address argument, requiring the count argument",
"              to be greater than 1 in order to display memory before the",
"              specified address.",
"",
"       -o offs",
"              offset the starting address by offs.",
"",
"       -e addr",
"              display memory until reaching specified ending hexadecimal",
"              address.",
"",
"       -r file",
"              dumps raw data to the specified output file; the number of bytes",
"              that are copied to the file must be specified either by a count",
"              argument or by the -e option.",
"",
...
"",
"       Display the same block of memory, first without symbols, again with",
"       symbols, and then with symbols and slab cache references:",
...
"",
"       -seconds",
"              The number of seconds to delay between command executions.  This",
"              option must precede the command name to be executed.",
"",
"       Command execution may be stopped with CTRL-C, or if scrolling is in",
"       effect, by entering \"q\".  This command is meant for use on a live",
"       system; it is hard to conceive of a reason to use it when debugging a",
"       crash dump.",
"",
"EXAMPLES",
...
"[-t] [-T] [-m] [-g] [-c cpu(s)]",
"NAME",
"       crash-runq - run queue",
"",
"SYNOPSIS",
"       runq [-t] [-T] [-m] [-g] [-c cpu(s)]",
"",
"DESCRIPTION",
"       With no argument, this command displays the tasks on the run queues of",
"       each cpu.",
"",
"       -t     Display the timestamp information of each cpu's runqueue, which",
"              is the rq.clock, rq.most_recent_timestamp or",
"              rq.timestamp_last_tick value, whichever applies; following each",
"              cpu timestamp is the last_run or timestamp value of the active",
"              task on that cpu, whichever applies, along with the task",
"              identification.",
"",
"       -T     Display the time lag of each CPU relative to the most recent",
"              runqueue timestamp.",
"",
"       -m     Display the amount of time that the active task on each cpu has",
"              been running, expressed in a format consisting of days, hours,",
"              minutes, seconds and milliseconds.",
"",
"       -g     Display tasks hierarchically by task_group.  The task_group line",
"              shows the task_group address, the cfs_rq or rt_rq address, the",
"              task_group name (if any), and whether the task_group is",
"              throttled.",
"",
...
"<symbol>",
"NAME",
"       crash-rustfilt - demangle a mangled Rust symbol to human readable symbol",
"",
"SYNOPSIS",
"       rustfilt <symbol>",
"",
...
"NAME",
"       crash-s390dbf - s390dbf prints out debug feature logs",
"",
"SYNOPSIS",
"       s390dbf [-v] [-s dirname] [debug log] [debug view]",
"",
"DESCRIPTION",
"       Display Debug logs: + If called without parameters, all active debug",
"       logs are listed.  + If called with the name of a debug log, all debug-",
"       views for which",
...
"NAME",
"       crash-sbitmapq - sbitmap_queue struct contents",
"",
"SYNOPSIS",
"       sbitmapq [-s struct[.member[,member]] -a address [-p] [-v]] -[x|d]",
"       address",
"",
"DESCRIPTION",
"       The command dumps the contents of the sbitmap_queue structure and the",
"       used bits in the bitmap. Also, it shows the dump of a structure array",
"       associated with the sbitmap_queue.",
"",
"       The arguments are as follows:",
"",
"       -s struct",
"              name of a C-code structure, that is stored in an array associated",
"              with sbitmap_queue structure. Use the \"struct.member\" format in",
"              order to display a particular member of the structure. -s option",
"              requires -a option",
"",
"       -a address",
"              address of a structure array associated with sbitmap_queue",
"              structure. The set bits in sbitmap are used for the index in an",
"              associated array.",
"",
...
"       -d     override default output format with decimal format.",
"",
"       -v     By default, the sbitmap command shows only a used sbitmap index",
"              and a structure address in the associated array.  This flag says",
"              to print a formatted display of the contents of a structure in an",
"              associated array. -v option requires of -s.",
"",
"EXAMPLES",
...
"           00000010: 00",
"",
"       Display the addresses of structure are associated with sbitmap_queue",
"       (for iscsi it is 'iscsi_cmd' structure):",
...
char *help_search[] = {
"@MANPAGE@",
"[-s start] [ -[kKV] | -u | -p | -t | -T ] [-e end | -l length] [-m mask] [-x count] -[cwh] [value | (expression) | symbol | string] ...",
"NAME",
"       crash-search - search memory",
"",
"SYNOPSIS",
"       search [-s start] [ -[kKV] | -u | -p | -t | -T ] [-e end | -l length]",
"       [-m mask] [-x count] -[cwh] [value | (expression) | symbol | string] ...",
"",
"DESCRIPTION",
"       This command searches for a given value within a range of user virtual,",
"       kernel virtual, or physical memory space.  If no end nor length value is",
"       entered, then the search stops at the end of user virtual, kernel",
"       virtual, or physical address space, whichever is appropriate.",
"",
"       An optional mask value may be entered to mask off \"don't care\" bits.",
"",
"       -s start",
"              Start the search at this hexadecimal user or kernel virtual",
...
"",
"       -k     If no start address is specified, start the search at the base of",
"              kernel virtual address space.  This option is the default.",
"",
...
"              searched.",
"",
"       -u     If no start address is specified, start the search at the base of",
"              the current context's user virtual address space.  If a start",
"              address is specified, then this option specifies that the start",
"              address is a user virtual address.",
"",
"       -p     If no start address is specified, start the search at the base of",
"              physical address space.  If a start address is specified, then",
"              this option specifies that the start address is a physical",
"              address.",
"",
...
"",
"       -l length",
"              Length in bytes of address range to search.",
"",
"       -m mask",
"              Ignore the bits that are set in the hexadecimal mask value.",
"",
...
"",
"       -x count",
"              Display the memory contents before and after any found value.",
"              The before and after memory context will consist of \"count\"",
"              memory items of the same size as the \"value\" argument.  This",
"              option is not applicable with the -c option.  value  Search for",
"              this hexadecimal long, unless modified by the -c, -w, or -h",
"              options.",
"",
"       (expression)  Search for the value of this expression; the expression",
"       value must",
...
"                     with the -c option.",
"",
"       If -k, -K, -V, -u, -p or -t are not used, then the search defaults to",
"       kernel virtual address space.  The starting address must be long-word",
"       aligned.  Address ranges that start in user space and end in kernel",
"       space are not accepted.",
"",
"EXAMPLES",
"       Search the current context's address space for all instances of",
"       0xdeadbeef:",
...
"           ...",
"",
"       Search kernel memory for all instances of 32-bit value 0xbabe1174 on a",
"       64-bit system:",
...
char *help_set[] = {
"@MANPAGE@",
"[[-a] [pid | taskp] | [-c cpu] | -p] | [crash_variable [setting]] | -v",
"NAME",
"       crash-set - set a process context or internal crash variable",
"",
"SYNOPSIS",
"       set [[-a] [pid | taskp] | [-c cpu] | -p] | [crash_variable [setting]] |",
"       -v",
"",
"DESCRIPTION",
...
"",
"       -a     sets the pid or task as the active task on its cpu (dumpfiles",
"              only).",
"",
...
"",
"       If no argument is entered, the current context is displayed.  The",
"       context consists of the PID, the task pointer, the CPU, and task state.",
"       The task state shows the bits found in both the task_struct state and",
"       exit_state fields.",
"",
"       This command may also be used to set internal crash variables.  If no",
"       value argument is entered, the current value of the crash variable is",
"       shown.  These are the crash variables, acceptable arguments, and",
"       purpose:",
...
"",
"       During initialization, $HOME/.crashrc is read first, followed by the",
"       .crashrc file in the current directory.  Set commands in the .crashrc",
"       file in the current directory override those in $HOME/.crashrc.  Set",
"       commands entered with this command or by runtime input file override",
"       those defined in either .crashrc file.  Multiple set command arguments",
"       or argument pairs may be entered in one command line.",
"",
"EXAMPLES",
...
"NAME",
"       crash-sig - task signal handling",
"",
"SYNOPSIS",
...
"       Multiple task or PID numbers may be entered; if no arguments are",
"       entered, the signal handling data of the current context will be",
"       displayed.  The default display shows:",
...
"             taskp  a hexadecimal task_struct pointer.",
"",
"       -g     displays signal information for all threads in a task's thread",
"              group.",
"",
"       -l     displays the defined signal numbers and names.",
"",
"       -s sigset",
"              translates a 64-bit hexadecimal value representing a sigset_t",
...
"                        64  f51b9500",
"",
"       Dump the signal-handling data for all tasks in the thread group",
"       containing PID 2578:",
"           crash> sig -g 2578",
...
"            ...",
"",
"       Translate the sigset_t mask value, cut-and-pasted from the signal",
"       handling data from signals 1 and 10 above:",
...
char *help_struct[] = {
"@MANPAGE@",
"struct_name[.member[,member]][-o][-l offset][-rfuxdp] [address | symbol][:cpuspec] [count | -c count]",
"NAME",
"       crash-struct - structure contents",
"",
"SYNOPSIS",
"       struct struct_name[.member[,member]][-o][-l offset][-rfuxdp] [address |",
"       symbol][:cpuspec] [count | -c count]",
"",
"DESCRIPTION",
"       This command displays either a structure definition, or a formatted",
"       display of the contents of a structure at a specified address.  When no",
"       address is specified, the structure definition is shown along with the",
"       structure size.  A structure member may be appended to the structure",
"       name in order to limit the scope of the data displayed to that",
"       particular member; when no address is specified, the member's offset and",
...
"",
"       -o     show member offsets when displaying structure definitions; if",
"              used with an address or symbol argument, each member will be",
"              preceded by its virtual address.",
"",
"       -l offset",
"              if the address argument is a pointer to a structure member that",
"              is contained by the target data structure, typically a pointer to",
"              an embedded list_head, the offset to the embedded member may be",
"              entered in either of the following manners: 1. in",
"              \"structure.member\" format.  2. a number of bytes.",
"",
...
"       -d     override default output format with decimal format.",
"",
"       -p     if a structure member is a pointer value, show the member's data",
"              type on the output line; and on the subsequent line(s),",
...
"",
"       -c count",
"              \"-c\" is only required if \"count\" is not the last argument entered",
"              or if a negative number is entered; if a negative value is",
"              entered, the (positive) \"count\" structures that lead up to and",
"              include the target structure will be displayed.",
"",
"       Structure data, sizes, and member offsets are shown in the current",
"       output radix unless the -x or -d option is specified.",
"",
"       Please note that in the vast majority of cases, the \"struct\" command",
"       name may be dropped; if the structure name does not conflict with any",
"       crash or gdb command name, then the \"struct_name[.member]\" argument will",
"       be recognized as a structure name, and this command automatically",
...
"           }",
"",
"       Display the definition and size of a vm_area_struct structure.  This",
"       first example below displays just the structure and size.  The second",
"       example uses the -o option to also display member offsets.  Both",
"       examples were run with the output radix set to 10 (decimal):",
...
"                }",
"",
"       Display the thread_info structure pointed to by the thread_info member",
"       of the task_struct at ffff8100181190c0:",
...
"                }",
"",
"       Display the flags and virtual members of 4 contiguous page structures in",
"       the mem_map page structure array:",
...
"             virtual = 0xc04b3000",
"",
"       Display the array of tcp_sl_timer structures declared by",
"       tcp_slt_array[]:",
...
"             },",
"",
"       Display the child dentry structure referenced by the \"next\" pointer",
"       above.  Since the \"next\" address of 0x3661344 above is a pointer to an",
"       embedded list_head structure within the child dentry structure, the -l",
"       option is required:",
...
"           ...",
"",
"       Display the embedded sched_entity structure's on_rq member and the third",
"       pid_link structure in the embedded pids[] array of the task_struct at",
"       ffff88011653e250:",
...
"             }",
"",
"       For an example of displaying per-cpu variables, consider the struct",
"       hd_struct.dkstats member, which is a percpu pointer to a disk_stats",
"       structure:",
...
"           }",
"",
"       Taking an hd_struct at address ffff8802450e2848, display all of the per-",
"       cpu disk_stats structures that it references:",
...
"       the need to enter the \"struct\" command name (enter \"help *\" for",
"       details).",
"",
"SEE ALSO",
"       crash(8)",
...
"",
"NAME",
"       crash-swap - swap device information",
"",
"SYNOPSIS",
"       swap",
"",
...
"",
"EXAMPLE",
"       crash> swap SWAP_INFO_STRUCT    TYPE       SIZE       USED    PCT  PRI",
"       FILENAME ffff880153d45f40  PARTITION  7192568k   1200580k  16%   -1",
"       /dev/dm-1",
"",
"SEE ALSO",
"       crash(8)",
...
"NAME",
"       crash-sym - translate a symbol to its virtual address, or vice-versa",
"",
"SYNOPSIS",
...
"       kernel virtual address to its symbol -- or to a symbol-plus-offset",
"       value, if appropriate.  Additionally, the symbol type is shown in",
"       parentheses, and if the symbol is a known text value, the file and line",
"       number are shown.",
"",
...
"",
"       -m module",
"              dumps the current set of symbols for a specified module.",
"",
...
"",
"       -q string",
"              searches for all symbols containing \"string\".  symbol  a kernel",
"              text or data symbol.  vaddr  a kernel virtual address.",
"",
...
"",
"       Dump the symbols of the uart401 module, both before, and then after, the",
"       complete set of symbols are loaded with the \"mod -s\" command:",
"           crash> sym -m uart401",
...
"       entered, the same system data shown during crash invocation is shown.",
"",
"       -c [name|number]",
"              If no name or number argument is entered, dump all sys_call_table",
"              entries.  If a name string is entered, search the table for all",
"              entries containing the string.  If a number is entered, the table",
"              entry associated with that number is displayed.  If the current",
"              output radix has been set to 16, the system call numbers will be",
"              displayed in hexadecimal.",
"                  config            If the kernel was configured with CONFIG_IKCONFIG, then",
"                                    dump the in-kernel configuration data.",
"",
"       -t     Display kernel taint information.  If the \"tainted_mask\" symbol",
"              exists, show its hexadecimal value and translate each bit set to",
"              the symbolic letter of the taint type.  On older kernels with the",
"              \"tainted\" symbol, only its hexadecimal value is shown.  The",
"              relevant kernel sources should be consulted for the meaning of",
"              the letter(s) or hexadecimal bit value(s).",
"",
"       -panic Panic a live system.  Requires write permission to /dev/mem.",
"              Results in the crash context causing an \"Attempted to kill the",
"              idle task!\" panic.  (The dump will indicate that the crash",
"              context has a PID of 0).",
"",
...
"NAME",
"       crash-task - task_struct and thread_info contents",
"",
"SYNOPSIS",
...
"       thread_info structures of the current context are displayed.  The -R",
"       option, which may also be invoked indirectly via \"foreach task\", pares",
"       the output down to one or more structure members.",
"               pid  a process PID.",
"             taskp  a hexadecimal task_struct pointer.",
"",
"       -R member",
"              a comma-separated list of one or more task_struct and/or",
"              thread_info structure members.  If any member contains an",
"              embedded structure, or is an array, the output may be restricted",
"              to the embedded structure or an array element by expressing the",
"              member argument as \"member.member\" or \"member[index]\"; embedded",
"              member specifications may extend beyond one level deep, by",
"              expressing the member argument as \"member.member.member...\".",
"",
"       -x     override default output format with hexadecimal format.",
"",
"       -d     override default output format with decimal format.",
...
"",
"       crash> task -R pids[2] PID: 6529   TASK: ffff880116538790  CPU: 0",
"       COMMAND: \"bash\"",
"           pids[2] =   {",
"           node = {",
...
"       }",
"",
"       NOTE: When this command is invoked directly (i.e., not from \"foreach\"),",
"       it is not necessary to include the \"-R\" before the",
"       task_struct/thread_info member name(s).",
"",
"SEE ALSO",
"       crash(8)",
NULL
...
char *help_tree[] = {
"@MANPAGE@",
"[-t [radix|xarray|rbtree|maple]] [-r offset] [-[s|S] struct[.member[,member]]] -[x|d] [-o offset] [-l] [-p] [-v] [-N] start",
"NAME",
"       crash-tree - display radix tree, XArray, red-black tree or maple tree",
"",
"SYNOPSIS",
"       tree [-t [radix|xarray|rbtree|maple]] [-r offset] [-[s|S]",
"       struct[.member[,member]]] -[x|d] [-o offset] [-l] [-p] [-v] [-N] start",
"",
"DESCRIPTION",
...
"",
"       -t type",
"              The type of tree to dump; the type string can be one of \"radix\",",
"              \"rbtree\", \"xarray\", or \"maple\", or alternatively, \"ra\", \"rb\", \"x\"",
"              or \"m\" are acceptable. If not specified, rbtree is the default",
"              type.",
"",
"       -r offset",
"              If the \"start\" argument is the address of a data structure that",
...
"              may be entered in either of two manners: 1. In \"structure.member\"",
"              format.  2. A number of bytes.",
"",
"       -o offset",
"              For red-black trees only, the offset of the rb_node within its",
...
"              manners: 1. In \"structure.member\" format.  2. A number of bytes.",
"              This option is not applicable to radix trees.",
"",
"       -s struct",
"              For each entry in a tree, format and print it as this type of",
"              data structure; use the \"struct.member\" format in order to",
"              display a particular member of the structure.  To display",
"              multiple members of a structure, use a comma-separated list of",
"              members.  If any structure member contains an embedded structure",
"              or is an array, the the output may be restricted to the embedded",
"              structure or an array element by expressing the member argument",
"              as \"struct.member.member\" or \"struct.member[index]\"; embedded",
"              member specifications may extend beyond one level deep by",
"              expressing the struct argument as",
...
"              starting with the leftmost node and progressing to the right.",
"              This option does not apply to radix trees.",
"",
...
"              indicates \"root/l/r\" means that the node is the right child of",
"              the left child of the root node.  For radix trees, xarrays and",
...
"",
"       -N start",
"              The address of a radix_tree_node, maple_node, xa_node or rb_node",
...
"           ...",
"",
"       Display the vmap_area's va_start and va_end members of each of the",
"       entries above expressing the vmap_area.rb_node offset as a number of",
"       bytes:",
...
"           ...",
"",
"       Alternatively, use the -N option with the rb_node address contained in",
"       the vmap_area_root structure:",
...
"           ffff88012d65c440",
"",
"       Display the virtual address of each vm_area_struct in the red-black tree",
"       that has its root inside an mm_struct located at ffff880128b5a300.  The",
"       vm_area_struct.vm_rb rb_node member has an offset of 0x38 bytes:",
...
"",
"       Given an mm_struct address of 0xffff880074b5be80, list the VMA tree in",
"       linear order from the leftmost node progressing to the right using the",
"       -l option:",
...
"       radix_tree_root structure in the address_space structure above, and",
"       display the tree with the -N option:",
...
"           ...",
"",
"       Using the same radix tree as above, display the flags and _count members",
"       of each page struct in the list, and force the output format to be",
"       hexadecimal:",
...
"       structure located at 0xffff94c235e76828, where the i_pages field is an",
"       embedded xarray structure:",
...
"",
"       Alternatively, take the value found in the xa_head field from the xarray",
"       structure, and display the tree with the -N option:",
...
"       members of each page struct in the list, and force the output format to",
"       be hexadecimal:",
...
"             }",
"",
"       The -v option is introduced specifically for dumping the complete",
...
char *help_union[] = {
"@MANPAGE@",
"union_name[.member[,member]] [-o][-l offset][-rfuxdp] [address | symbol][:cpuspec] [count | -c count]",
"NAME",
"       crash-union - union contents",
"",
"SYNOPSIS",
"       union union_name[.member[,member]] [-o][-l offset][-rfuxdp] [address |",
"       symbol][:cpuspec] [count | -c count]",
"",
"DESCRIPTION",
...
"       specified, the union definition is shown along with the union size.  A",
"       union member may be appended to the structure name in order to limit the",
"       scope of the data displayed to that particular member; when no address",
...
"",
"       -o     show member offsets when displaying union definitions; the offset",
"              is always 0 unless used with an address or symbol argument, in",
"              which case each member will be preceded by its virtual address.",
"",
"       -l offset",
"              if the address argument is a pointer to a list_head structure",
"              that is embedded in the target union structure, the offset to the",
"              list_head member may be entered in either of the following",
"              manners: 1. in \"structure.member\" format.  2. a number of bytes.",
"",
...
"       -d     override default output format with decimal format.",
"",
"       -p     if a union member is a pointer value, show the member's data type",
"              on the output line; and on the subsequent line(s), dereference",
"              the pointer, display the pointer target's symbol value in",
"              brackets if appropriate, and if possible, display the target",
"              data; requires an address argument.",
"",
"       -u     address argument is a user virtual address in the current",
"              context.",
...
"              include the target structure will be displayed.",
"",
"       Union data, sizes, and member offsets are shown in the current output",
"       radix unless the -x or -d option is specified.",
"",
"       Please note that in the vast majority of cases, the \"union\" command name",
"       may be dropped; if the union name does not conflict with any crash or",
"       gdb command name, then the \"union_name[.member]\" argument will be",
...
"",
"       Lastly, the short-cut \"*\" (pointer-to) command may also be used to",
"       negate the need to enter the \"union\" command name (enter \"help *\" for",
"       details).",
"",
"SEE ALSO",
"       crash(8)",
...
char *help_vm[] = {
"@MANPAGE@",
"[-p | -P vma | -M mm | -v | -m | -x | -d | [-R reference] [pid | task]] [-f vm_flags]",
"NAME",
"       crash-vm - virtual memory",
"",
"SYNOPSIS",
"       vm [-p | -P vma | -M mm | -v | -m | -x | -d | [-R reference] [pid |",
"       task]] [-f vm_flags]",
"",
"DESCRIPTION",
...
"       total virtual memory size; and a list of pointers to each",
"       vm_area_struct, its starting and ending address, vm_flags value, and",
"       file pathname.  If no arguments are entered, the current context is",
"       used.  Additionally, the -p option translates each virtual page of each",
"       VM area to its physical address.  The -R option, typically invoked from",
"       \"foreach vm\", searches for references to a supplied number, address, or",
"       filename argument, and prints only the essential information leading up",
"       to and including the reference.  Alternatively, the -m or -v options may",
"       be used to dump the task's mm_struct or all of its vm_area_structs",
"       respectively.  The -p, -v, -m, -R and -f options are all mutually",
"       exclusive.",
"",
"       -p     translate each virtual page to its physical address, or if the",
"              page is not mapped, its swap device and offset, or filename and",
"              offset.",
"",
"       -P vma similar to -p, but only translate the pages belonging to the",
"              specified VM area of a context.",
"",
"       -M mm  if the mm_struct address has been removed from the task_struct of",
"              an exiting task, the virtual memory data cannot be displayed.",
...
"              virtual memory data of the task.  NOTE: this option is only used",
"              when the task's mm_struct is NULL.",
"",
"       -R reference",
"              search for references to this number or filename.",
"",
"       -m     dump the mm_struct associated with the task.",
"",
"       -v     dump all of the vm_area_structs associated with the task.",
"",
"       -x     override the default output format for the -m or -v options with",
"              hexadecimal format.",
"",
"       -d     override the default output format for the -m or -v options with",
"              decimal format.",
"",
"       -f vm_flags",
"              translate the bits of a FLAGS (vm_flags) value.  pid  a process",
"              PID.  task  a hexadecimal task_struct pointer.",
"",
"EXAMPLES",
...
"",
"       Display the virtual memory data along with page translations for PID",
"       386:",
"           crash> vm -p 386",
...
"       executed directly.  This example displays all VM areas with vm_flags of",
"       75:",
"           crash> vm -R 75",
...
"       the -p option on a task with a huge address space.  This example",
"       displays the page data associated with virtual address 40121000:",
"           crash> vm -R 40121000",
...
"NAME",
"       crash-vtop - virtual to physical",
"",
"SYNOPSIS",
...
"       physical page, and the swap location or file location if the page is not",
"       mapped.  The -u and -k options specify that the address is a user or",
"       kernel virtual address; -u and -k are not necessary on processors whose",
"       virtual addresses self-define themselves as user or kernel.  User",
"       addresses are translated with respect to the current context unless the",
"       -c option is used.  Kernel virtual addresses are translated using the",
"       swapper_pg_dir as the base page directory unless the -c option is used.",
"",
"       -u     The address is a user virtual address; only required on",
"              processors with overlapping user and kernel virtual address",
"              spaces.",
"",
"       -k     The address is a kernel virtual address; only required on",
"              processors with overlapping user and kernel virtual address",
"              spaces.",
"",
"       -c [pid | taskp]",
"              Translate the virtual address from the page directory of the",
"              specified PID or hexadecimal task_struct pointer.  However, if",
"              this command is invoked from \"foreach vtop\", the pid or taskp",
"              argument should NOT be entered; the address will be translated",
"              using the page directory of each task specified by \"foreach\".",
...
"",
"       Translate kernel virtual address c806e000, first using swapper_pg_dir as",
"       the page directory base, and secondly, using the page table base of PID",
"       1359:",
"           crash> vtop c806e000",
"",
"       VIRTUAL PHYSICAL",
...
"NAME",
"       crash-waitq - list tasks queued on a wait queue",
"",
"SYNOPSIS",
"       waitq [ symbol ] | [ struct.member struct_addr ] | [ address ]",
"",
"DESCRIPTION",
"       This command walks the wait queue list displaying the tasks which are",
"       blocked on the specified wait queue.  The command differentiates between",
"       the old- and new-style wait queue structures used by the kernel.  It can",
"       be invoked with the following argument types:",
"                            symbol  a global symbol of a wait queue.",
"",
"       struct.member struct_addr  a structure name and wait queue member",
"       combination",
...
char *help_whatis[] = {
"@MANPAGE@",
"[[-o] [struct | union | typedef | symbol]] | [[-r [size|range]] [-m member]]",
"NAME",
"       crash-whatis - search symbol table for data or type information",
"",
"SYNOPSIS",
"       whatis [[-o] [struct | union | typedef | symbol]] | [[-r [size|range]]",
"       [-m member]]",
"",
"DESCRIPTION",
...
"                  symbol  a kernel symbol.",
"",
"       Alternatively, a search can be made for data structures of a given size",
"       or size range, that contain a member of a given type, or contain a",
"       pointer to given type.  The -r and -m options may be used alone or in",
"       conjunction with one another:",
"",
"       -r size",
"              search for structures of this exact size.",
"",
"       -r range",
"              search for structures of a range of sizes, expressed as \"low-",
"              high\".",
"",
"       -m member",
"              search for structures that contain a member of this data type, or",
"              that contain a pointer to this data type; if a structure contains",
"              another structure, the members of the embedded structure will",
"              also be subject to the search. The member argument may also be",
"              expressed as a substring of a member's data type.",
"",
"EXAMPLES",
...
"            192  urb",
"",
"       Display all structures that contain members that point to an mm_struct:",
...
"           2936  task_struct",
"",
"       Display all structures sized from 256 to 512 bytes that contain members",
"       that point to a task_struct:",
...
"NAME",
"       crash-wr - write memory",
"",
"SYNOPSIS",
"       wr [-u|-k|-p] [-8|-16|-32|-64] [address|symbol] value",
"",
"DESCRIPTION",
"       This command modifies the contents of memory.  The starting address may",
"       be entered either symbolically or by address.  The default modification",
"       size is the size of a long data type.  Write permission must exist on",
"       the /dev/mem.  When writing to memory on a live system, this command",
"       should obviously be used with great care.",
"",
...
"",
"       -p option is used.",
"              If a virtual address is specified, the",
"",
